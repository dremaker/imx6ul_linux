...
static irqreturn_t sim_irq_handler(int irq, void *dev_id)
{
	...
	if (sim->state == SIM_STATE_ATR_RECEIVING &&
		...
		if ((tx_status & SIM_XMT_STATUS_GPCNT) &&
			...
			pr_debug("atr receving  gpcnt time out irq \n");
		...
		if ((rx_status & SIM_RCV_STATUS_CWT) ||
			((tx_status & SIM_XMT_STATUS_GPCNT) &&
				(sim->rcv_count != 0))) {
			pr_debug("atr receving  cwt or gpcnt irq \n");
			/*Disable the GPCNT timer and CWT timer right now*/
			reg_data = __raw_readl(sim->ioaddr + CNTL);
	 ...
	 */
		...
		if ((rx_status & SIM_RCV_STATUS_CWT) ||
			...
			(tx_status & SIM_XMT_STATUS_GPCNT)){
			pr_debug("reset sequency status cpcnt irq tx_status 0x%X\n", tx_status);
			complete(&sim->xfer_done);
		}	
	else if (rx_status & SIM_RCV_STATUS_RDRF) {
...
static void sim_start(struct sim_t *sim)
{
	...
	/*in fact, we do not use port detect*/
#ifdef SIM_USING_PORT_DETECT
...
static void sim_polling_delay(struct sim_t *sim, u32 delay)
{
	...
	/*Clear the interrupt status   ÓÐÎÊÌâ°¡*/
...
static void sim_debug(void __iomem * ioaddr)
{
	int i;
	u32 reg_data;
	for(i=0; i<31; i++)
	{
		if(i*4 == PORTO_XMT_BUF || i*4 == PORT0_RCV_BUF)
			continue;
 		reg_data = __raw_readl(ioaddr + i*4);
		pr_debug("address: 0x%08X, value: 0x%08X \n", (u32)ioaddr + i*4, reg_data);
	}
}
		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	case SIM_IOCTL_DEBUG_PRINT:
		pr_debug("SIM_IOCTL_GET_PRESENSE %x\n", SIM_IOCTL_GET_PRESENSE);
		pr_debug("SIM_IOCTL_GET_ATR %x\n", SIM_IOCTL_GET_ATR);
		pr_debug("SIM_IOCTL_XMT %x\n", SIM_IOCTL_XMT);
		pr_debug("SIM_IOCTL_RCV %x\n", SIM_IOCTL_RCV);
		pr_debug("SIM_IOCTL_ACTIVATE %x\n", SIM_IOCTL_ACTIVATE);
		pr_debug("SIM_IOCTL_DEACTIVATE %x\n", SIM_IOCTL_DEACTIVATE);
		pr_debug("SIM_IOCTL_WARM_RESET %x\n", SIM_IOCTL_WARM_RESET);
		pr_debug("SIM_IOCTL_COLD_RESET %x\n", SIM_IOCTL_COLD_RESET);
		pr_debug("SIM_IOCTL_CARD_LOCK %x\n", SIM_IOCTL_CARD_LOCK);
		pr_debug("SIM_IOCTL_CARD_EJECT %x\n", SIM_IOCTL_CARD_EJECT);
		pr_debug("SIM_IOCTL_SET_PROTOCOL %x\n", SIM_IOCTL_SET_PROTOCOL);
		pr_debug("SIM_IOCTL_SET_TIMING %x\n", SIM_IOCTL_SET_TIMING);
		pr_debug("SIM_IOCTL_SET_BAUD %x\n", SIM_IOCTL_SET_BAUD);
		pr_debug("SIM_IOCTL_WAIT %x\n", SIM_IOCTL_WAIT);
		pr_debug("SIM_IOCTL_DEBUG_PRINT %x\n", SIM_IOCTL_DEBUG_PRINT);
		sim_debug(sim->ioaddr);
		break;
...
copy_data:
	...
	default:
		errval = SIM_E_PARAM_CMD_INVALID;
		break;
